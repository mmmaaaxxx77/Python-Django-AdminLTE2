...
class MethodRouter(object):
    ...
    def __call__(self, django_request, **url_parameters):
        ...
        query_args = {}#django_request.GET
        for key in django_request.GET:
            try:
                query_args[str(key)] = int(django_request.GET[key][0])
                break;
            except:
                pass
            try:
                query_args[str(key)] = str(django_request.GET[key][0])
                break;
            except:
                pass

        request = DjangoRequestContainer(django_request, url_params=url_parameters,
                                         query_args=query_args, body_args=dict(body_parameters),
